...
saveId.addEventListener("change",function(event){
    ...
    // 체크박스.checked : 체크 O == ture, 체크X == false 반환

    // 체크박스.checked=true||false
    // -> true 대입 : 체크 O
    //    false 대입 : 체크 X
    if(this.checked){
        const str="개인정보보호를 위해 개인PC에서의 사용을 권장합니다."
                +"개인 PC가 아닌 경우 해제 해주세요";
        // window.confirm("내용"): 확인==true, 취소==false반환        
...
function loginValidate(){
    ...
    // 이메일이 입력되지 않은 경우 false반환
    if(memberEmail.value.trim().length==0){
        // 이메일 양쪽 공백을 제거한 후 길이가 0인 경우
        // == 이메일 미작성
        alert("이메일을 입력해주세요");
        memberEmail.focus(); // 이메일 input요소에 초점을 맞춤
        memberEmail.value=""; // 이메일 input요소에 작성된 값을 모두 삭제
        return false;
    }
    if(memberPw.value.trim().length==0){
        ...
        // == 비밀번호 미작성
        alert("비밀번호를 입력해주세요");
        memberPw.focus(); // 이메일 input요소에 초점을 맞춤
        memberPw.value=""; // 이메일 input요소에 작성된 값을 모두 삭제
        return false;
    }
}
...
function selectMemberList(){
    ...
    $.ajax({
        ...
        dataType:"JSON", // 응답데이터 JSON -> 자동으로 JS객체로 변환
        success:memberList=>{
