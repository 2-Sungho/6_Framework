...
// (true : 업로드 된 이미지 있음, false : 기본 이미지)
let initCheck;

// 이미지가 업로드 되었거나 삭제되었음을 나타내는 변수
// (-1: 초기값(취소), 0: 프로필 삭제(X버튼 클릭, 취소),1: 새 이미지 업로드 )
let deleteCheck=-1;
    ...
    imageInput.addEventListener("change",(e)=>{
        ...
        if(e.target.files[0]!=undefined){
            ...
            reader.onload=event=>{
                ...
                // img태그의 src속성으로 읽은 이미지 파일 경로 추가
                // == 이미지 경로 추가
