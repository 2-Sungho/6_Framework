...
public class MemberServiceImpl implements MemberService {
	...
	// MemberDAO bean을 의존성주입(DI)
	@Autowired
	private MemberDAO dao;
	
	// spring-security.xml에서 등록한 bean을 
	// 의존성 주입(DI)
	@Autowired
...
//		System.out.println("암화화 비밀번호: "+bcrypt.encode(inputMember.getMemberPw()));
	 ...
	 * 	  - 클래스, 메서드 위에 작성 가능(주로 메서드 위에)
	 * 	  - 조건 : AOP를 이용한 기술 -> Service Interface 필요
	 * 			  트랜잭션 매니저가 bean으로 등록되어 있어야함(root-context.xml)
	 * 
	 * @Transactional 어노테이션 특징
	 * - 예외가 발생한 경우 rollback 자동 진행
	 * - 예외의 기본값은 RuntimeException
	 * 		-> SQL 진행 시 발생하는 예외 == SQLException(RuntimeException 형제) 다형성 적용X
	 * 		-> 다른 예외에도 rollback이 진행되고 싶다면 rollbackFor=예외클래스 작성하면된다
	 * 
	 * */
