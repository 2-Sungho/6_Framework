...
//	@RequestMapping(value = "/member/login", method = RequestMethod.POST)
	...
	public String login(/* @ModelAttribute */ Member inputMember, Model model, 
		...
		// Model : 데이터 전달용 객체
		// - 데이터를 Map형식(Key:Value)으로 저장하여 전달하는 객체
		// - request scope가 기본값
		...
		String path=null; // redirect 경로를 저장할 변수
		...
		if(loginMember!=null) {
			...
			// 기존 : HttpServletRequset req; req.getHeader("referer");
...
//			model.addAttribute("message", "아이디 또는 비밀번호가 일치하지 않습니다");
	...
	public String signUp(Member inputMember /* 커맨드 객체 */,
		...
		// 한글이 깨지는 이유
		// -> POST 요청 시 인코딩 처리 필요(web.xml 참고)
