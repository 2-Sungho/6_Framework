...
// 인터셉터
// 클라이언트 <-> 필터 <-> Dispather Servlet <-> *인터셉터* <-> 컨트롤러
...
public class BoardTypeInterceptor implements HandlerInterceptor {
	...
	// preHandle(전 처리) : 컨트롤러 호출 전에 수행
	// postHandle( 후 처리) : 컨트롤러 리턴 후 수행
	
	// afterCompletion(View단 처리 후)
	// 모든 뷰에서 최종결과를 생성하는 일을 포함한 모든 작업이 완료된 후 수행
